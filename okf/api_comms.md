...
2. **WebSocket Action Emission Standard**:
   ...
   - **Every action is sent with [`sendAction`](file:///c:/Fred/Coding/SK/expo-app/services/actions.ts)** — never `wsService.emit('action', …)` or `emitAction`. `npm run check:actions` in `expo-app/` fails on either. `sendAction(type, payload)` is typed from the protocol map and resolves to `{ ok: true, data }` or `{ ok: false, message, noAnswer? }`.
   - **The three reply formats differ.** An `action` is answered `{ status: 'ok', data }` or `{ status: 'error', message }` (`ActionAck` in `@sk/shared`, which the server's exit is typed against) — the result is *in* `data`. `get_data` answers the data unwrapped, or `{ status: 'error' }` when refused. REST answers the body, and `apiFetch` throws on a non-2xx. The socket wrapper answers `null` on a timeout or with no connection, and that is a failure whose outcome is unknown.
   - **Retries do not duplicate, and failures are recorded.** Every action carries a `requestId` the server replays rather than re-applies (`runIdempotent`, per user; in memory — `SYNC-5`). Every refusal, and every no-answer or unreadable reply the client reports on the `client_failures` socket event, is written to `server/logs/failures-YYYY-MM-DD.jsonl` for analysis. An action that changed nothing is refused at the handler's exit rather than answered `ok` with `null`.
   - **No silent failures, and a write that failed must never look like one that worked.** `sendAction` announces every failure (toast, unless the caller shows it inline with `suppressToast`) and logs it with the action type. On failure a caller does **not** perform the success consequences — no clearing the dirty state, no resetting the form, and above all **no navigating away**. The full rule, with the four ways the reply was being misread when this was introduced, is the [action-replies skill](file:///c:/Fred/Coding/SK/.agent/skills/action-replies/SKILL.md). History: `LIVE-19` (2026-09-15) made every emit pass a callback; the 2026-09-19 sweep found that was not enough — callbacks read the ack as the result, treated `null` as success, or hedged with `res?.data || res` — and moved all ~115 call sites to `sendAction`.
...
*   **Sport Age Groups**: the official list is written by admins through `/api/admin/sports/:id/age-groups` (list with usage counts, add, reorder) and `/api/admin/age-groups/:id` (rename/promote/demote, delete, and `POST …/merge`), each answering with the sport's whole list. Users add custom entries with the `ADD_AGE_GROUP` socket action, which returns the existing entry when the name already exists. Pickers read the list from `get_data: sports` (`sport.ageGroups`); `getSport` leaves it off because it sits on the scoring path. All in [AgeGroupManager.ts](file:///c:/Fred/Coding/SK/server/src/managers/AgeGroupManager.ts); see [api_actions.md](file:///c:/Fred/Coding/SK/docs/api_actions.md#age-groups).
