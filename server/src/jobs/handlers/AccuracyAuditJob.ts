...
export const accuracyAuditJob = async () => {
    ...
    try {
        ...
        for (const row of res.rows) {
            ...
            const description = `Corrected counts for ${name}: Teams (${team_count} -> ${actualTeams}), Members (${member_count} -> ${actualMembers}), Sites (${site_count} -> ${actualSites})`;

            await reportManager.submitReport({
                reporterUserId: null, // system-generated: no reporting user
    ...
    // 3. Notify Global Admins (broad alert) once the corrections are committed
