...
- **Except where the work does not happen in one sitting.** A tournament's name and dates are known in March, its entrants confirm through April, its fixtures follow, and its schedule is done the week before — so a wizard that must be completed before it produces anything is the wrong container. There, creation is reduced to the least that makes a real record (for a tournament, a name and a date — U45) and the screen carries a **setup checklist**: a resumable state that says what is outstanding and puts each remaining action in context. Steps that do not apply must be dismissible, or somebody who genuinely wants no points system is nagged about it forever.
- **A checklist hub is not a wizard, and the difference is worth naming.** Both show numbered-looking steps; they behave oppositely. A **wizard** is ordered, gates progress on the current step, and saves once at the end — correct when the data is only valid as a set. A **checklist hub** is a page of steps, each opening its own screen, in any order, each **saving itself**, with a step marked done by the state of the data rather than by having been visited — correct when the work is resumable and partial answers are legitimate. Do not put a step-nav across the top of a hub: it duplicates the checklist, and it does not fit a phone. Back goes to the hub, never to the previous step; `Next` names where it goes, so it reads as an offer rather than a sequence. See U48 in [tournaments-ui.md](file:///c:/Fred/Coding/SK/docs/tournaments-ui.md) §7.
