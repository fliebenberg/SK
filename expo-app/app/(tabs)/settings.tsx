...
export default function SettingsScreen() {
  ...
  const removeLocalOverride = useSettingsStore(
    (state) => state.removeLocalOverride,
  );
  const globalTheme = useSettingsStore(
    (state) => state.globalPreferences.theme,
  );
  ...
  const [socialAccounts, setSocialAccounts] = useState<
    Array<{ provider: string; provider_image: string }>
  >([]);
  const [emails, setEmails] = useState<
    Array<{ email: string; isPrimary: boolean; verifiedAt: string | null }>
  >([]);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && token) {
      ...
      apiService
        ...
        .catch((err) =>
          console.error("[Settings] Error fetching profile data:", err),
        )
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      if (res.success) {
        Alert.alert(
          "Success",
          user?.hasPassword ? "Password updated!" : "Password set!",
        );
        setOldPassword("");
        setNewPassword("");
        setConfirmPassword("");
