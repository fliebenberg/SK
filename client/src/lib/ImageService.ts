...
export class ImageService {
     ...
     */
    async processAvatar(base64: string, userId: string): Promise<string> {
        // Remove data:image/...;base64, prefix if present
        const base64Data = base64.replace(/^data:image\/\w+;base64,/, "");
        const buffer = Buffer.from(base64Data, 'base64');

        const baseFilename = `avatar-${userId}-${Date.now()}`;
     ...
     */
    async deleteAvatar(baseFilename: string) {
        ...
        for (const tier of tiers) {
            ...
            try {
                ...
                console.error(`Failed to delete avatar file: ${filePath}`, err);
            }
        }
    }
}
