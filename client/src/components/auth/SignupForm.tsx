...
export function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If there's an image, update the profile after signup
      if (image) {
          // This assumes signup logs the user in automatically
          // which my AuthContext implementation does.
          // Wait, signup might take a moment to reflect the session.
      }
      router.push('/');
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start space-x-2">
            <input
              ...
              className="h-4 w-4 rounded border-input bg-background accent-primary cursor-pointer mt-1"
            />
            <label
              htmlFor="terms"
